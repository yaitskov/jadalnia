...
import static org.dan.jadalnia.app.order.MatchInfo.MID;
...
import static org.dan.jadalnia.jooq.Tables.SET_SCORE;
...
import org.dan.jadalnia.jooq.tables.records.MatchesRecord;
import org.dan.jadalnia.sys.db.DbUpdateSql;
...
public class MatchDaoServer implements MatchDao {
    ...
    @Override
    public void createGroupMatch(DbUpdater batch, Oid oid, Fid fid, Gid gid, Cid cid, int priorityGroup,
        ...
        batch.exec(DbUpdateSql.builder()
                .query(jooq.insertInto(MATCHES, MATCHES.MID, MATCHES.TID,
                        ...
                        .values(oid, fid, Optional.of(gid).map(Gid::intValue), cid, place, priorityGroup,
                                Grup, bid1, bid2, tag.orElse(null)))
                .onFailure((u) -> internalError("Create group match", MID, oid))
                .mustAffectRows(Optional.of(1))
    ...
    @Override
    public void createPlayOffMatch(DbUpdater batch, Oid oid, Fid fid, Cid cid,
            Optional<Oid> winMid, Optional<Oid> loseMid,
        ...
        batch.exec(DbUpdateSql.builder()
                .query(jooq.insertInto(MATCHES, MATCHES.MID, MATCHES.TID, MATCHES.CID,
                        ...
                        .values(oid, fid, cid, priority, draft, winMid, loseMid, level,
                                type, tag.orElse(null)))
                .onFailure((u) -> internalError("Create play off match", MID, oid))
                .mustAffectRows(Optional.of(1))
    ...
    @Override
    public void scoreSet(Festival tournament, MatchInfo matchInfo,
    ...
    private void insertSetScore(DbUpdater batch, Fid fid, Oid oid, List<IdentifiedScore> scores) {
        scores.forEach(score -> batch.exec(
                DbUpdateSql.builder()
                        .query(jooq.insertInto(SET_SCORE, SET_SCORE.TID, SET_SCORE.MID,
                                SET_SCORE.BID, SET_SCORE.GAMES)
                                .values(fid, oid, score.getBid(), score.getScore()))
                        .build()));
    }
    ...
    @Override
    public void deleteByIds(Fid fid, Collection<Oid> oids, DbUpdater batch) {
        if (oids.isEmpty()) {
            return;
        }
        log.info("Remove mids {}", oids);
        batch
                .exec(DbUpdateSql.builder()
                        .mustAffectRows(empty())
                        .query(jooq.deleteFrom(SET_SCORE)
                                .where(SET_SCORE.TID.eq(fid),
                                        SET_SCORE.MID.in(oids)))
                        .build())
                .exec(DbUpdateSql.builder()
                        .mustAffectRows(NON_ZERO_ROWS)
                        .query(jooq.deleteFrom(MATCHES)
                                .where(MATCHES.TID.eq(fid),
                                        MATCHES.MID.in(oids)))
                        .build());
    }

    @Override
    public void deleteAllByTid(Festival tournament, DbUpdater batch, int size) {
        batch
                .exec(DbUpdateSql.builder()
                        .mustAffectRows(empty())
                        .query(jooq.deleteFrom(SET_SCORE)
    ...
    @Override
    public List<MatchInfo> load(Fid fid, MaxValue<Oid> maxMid) {
