...
import static org.dan.jadalnia.app.bid.BidState.Expl;
...
import static org.dan.jadalnia.app.bid.BidState.Rest;
import static org.dan.jadalnia.app.bid.BidState.WIN_STATES;
...
import static org.dan.jadalnia.app.order.OrderResource.UID;
...
import static org.dan.jadalnia.app.order.MatchState.INCOMPLETE_MATCH_STATES;
import static org.dan.jadalnia.app.order.MatchState.Over;
import static org.dan.jadalnia.app.order.MatchState.Place;
import static org.dan.jadalnia.app.order.MatchType.Gold;
import static org.dan.jadalnia.app.order.MatchType.Grup;
import static org.dan.jadalnia.app.place.ArenaDistributionPolicy.NO;
import static org.dan.jadalnia.app.playoff.PlayOffService.roundPlayOffBase;
import static org.dan.jadalnia.app.festival.ParticipantMemState.FILLER_LOSER_BID;
import static org.dan.jadalnia.app.festival.SetScoreResultName.MatchContinues;
import static org.dan.jadalnia.app.festival.FestivalCache.TOURNAMENT_RELATION_CACHE;
...
import org.dan.jadalnia.app.bid.SelectedBid;
import org.dan.jadalnia.app.bid.Uid;
import org.dan.jadalnia.app.category.SelectedCid;
import org.dan.jadalnia.app.festival.Fid;
import org.dan.jadalnia.app.group.Gid;
import org.dan.jadalnia.app.group.GroupDao;
import org.dan.jadalnia.app.group.GroupInfo;
import org.dan.jadalnia.app.group.GroupService;
import org.dan.jadalnia.app.group.PlayOffMatcherFromGroup;
...
import org.dan.jadalnia.app.sched.TablesDiscovery;
import org.dan.jadalnia.app.sport.MatchRules;
import org.dan.jadalnia.app.sport.Sports;
import org.dan.jadalnia.app.table.TableInfo;
import org.dan.jadalnia.app.festival.ChildTournamentProvider;
import org.dan.jadalnia.app.festival.ConfirmSetScore;
import org.dan.jadalnia.app.festival.ParticipantMemState;
import org.dan.jadalnia.app.festival.RelatedTids;
import org.dan.jadalnia.app.festival.SetScoreResultName;
import org.dan.jadalnia.app.festival.FestivalCache;
import org.dan.jadalnia.app.festival.Festival;
import org.dan.jadalnia.app.festival.TournamentProgress;
import org.dan.jadalnia.app.festival.FestivalService;
import org.dan.jadalnia.app.festival.TournamentTerminator;
import org.dan.jadalnia.app.festival.console.ConsoleStrategy;
import org.dan.jadalnia.app.user.UserState;
import org.dan.jadalnia.sys.db.DbUpdater;
import org.dan.jadalnia.util.TriFunc;
...
public class OrderService {
    ...
    public Optional<Bid> resolveUidAndMidToBid(Festival tournament,
            Uid uid, MatchInfo mInfo) {
        return ofNullable(tournament.getUidCid2Bid().get(uid))
                .flatMap(m -> ofNullable(m.get(mInfo.getCid())));
    }

    public SetScoreResult scoreSet(Festival tournament, Uid uid,
    ...
    private Optional<Bid> applyAtomicSetScore(Festival tournament, SetScoreReq score,
    ...
    SetScoreResult setScoreResult(
            Festival tournament, SetScoreResultName name, MatchInfo matchInfo) {
        switch (name) {
    ...
    public void matchWinnerDetermined(Festival tournament, MatchInfo mInfo,
    ...
    private void completePlayOffMatch(Festival tournament, MatchInfo mInfo,
    ...
    private void enlistIf(Festival tournament, ParticipantMemState par,
    ...
    public void tryToCompleteGroup(Festival tournament, MatchInfo matchInfo,
    ...
    public void tryToCompleteGroup(Festival tournament, Gid gid, DbUpdater batch) {
    ...
    private void completeParticipationLeftBids(Gid gid, Festival tournament,
    ...
    private void completeMiniTournamentGroup(
            Festival tournament, GroupInfo iGru,
    ...
    private Set<Bid> completeGroup(Gid gid, Festival tournament,
    ...
    private List<MatchInfo> playOffMatches(Festival tournament, GroupInfo iGru,
    ...
    private List<Bid> selectQuitingUids(Gid gid, Festival tournament,
    ...
    private void nextMatch(
            MatchInfo mInfo, Festival tournament, DbUpdater batch, Bid bid) {
    ...
    private void nextMatch(Festival tournament, Optional<Oid> nMid,
    ...
    private void bidRewalksLadder(Festival tournament,
            MatchInfo mInfo, DbUpdater batch, Bid bid) {
        final ParticipantMemState participant = tournament.getBidOrQuit(bid);
    ...
    public void assignBidToMatch(Festival tournament, Oid oid, Bid bid, DbUpdater batch) {
        log.info("Assign uid {} to mid {} in tid {}", bid, oid, tournament.getTid());
        final MatchInfo mInfo = tournament.getMatchById(oid);
    ...
    private void autoWinComplete(Festival tournament, MatchInfo mInfo,
            Bid winBid, DbUpdater batch) {
    ...
    private void checkPermissions(Festival tournament,
    ...
    public MatchScore matchScore(Festival tournament, MatchInfo matchInfo) {
    ...
    private Map<Bid, Integer> wonSets(Festival tournament, MatchInfo matchInfo) {
    ...
    public List<MatchInfo> bidIncompleteGroupMatches(Bid bid, Festival tournament) {
    ...
    public void walkOver(Festival tournament, Bid walkoverBid,
    ...
    public Optional<MatchInfo> playOffMatchForResign(Bid bid, Festival tournament) {
    ...
    public List<OpenMatchForWatch> findOpenMatchesForWatching(Festival tournament) {
    ...
    private LoadingCache<Fid, RelatedTids> relatedTidCache;

    @Inject
    private FestivalCache festivalCache;

    public Collection<Bid> findAllBidsOfUid(Festival tournament, Uid uid) {
        return ofNullable(tournament.getUidCid2Bid().get(uid))
                .map(Map::values)
    ...
    private TournamentProgress tournamentProgress(Festival tournament, Bid bid) {
    ...
    private TournamentProgress tournamentProgress(Festival tournament) {
        return TournamentProgress.builder()
    ...
    public List<OpenMatchForJudge> findOpenMatchesFurJudge(Festival tournament) {
        return scheduleService.withPlaceTables(tournament,
    ...
    public OpenMatchForJudgeList findOpenMatchesForJudgeList(Festival tournament) {
    ...
    public PlayedMatchList findPlayedMatchesByBid(Festival tournament, Bid bid) {
    ...
    public MatchResult matchResult(Festival tournament, Oid oid, Optional<Uid> ouid) {
        final MatchInfo m = tournament.getMatchById(oid);
    ...
    public UserState detectRole(Festival tournament, MatchInfo match, Optional<Uid> ouid) {
        return ouid.map(uid -> {
    ...
    public OpenMatchForJudge getMatchForJudge(Festival tournament, Oid oid) {
        return scheduleService.withPlaceTables(tournament,
                tablesDiscovery -> getMatchForJudge(tournament,
                        tournament.getMatchById(oid), tablesDiscovery));
    }

    private OpenMatchForJudge getMatchForJudge(
            Festival tournament, MatchInfo m,
    ...
    public List<Oid> findMatchesByParticipants(Festival tournament,
            Bid bid1, Bid bid2) {
        return tournament.getMatches().values().stream()
    ...
    public Oid createPlayOffMatch(SelectedCid sCid,
            Optional<Oid> winMid, Optional<Oid> loserMid,
            int priority, int level, MatchType type,
            Optional<MatchTag> oTag) {
        final Festival tournament = sCid.tournament();
        final Oid oid = tournament.getNextMatch().next();

        matchDao.createPlayOffMatch(
                sCid.batch(), oid, sCid.tid(), sCid.cid(), winMid, loserMid,
                priority, level, type, oTag, Draft);
        tournament.getMatches().put(oid, MatchInfo.builder()
                .tid(tournament.getTid())
                .mid(oid)
        ...
        log.info("playoff {} mid {} of tid {} in cid {}",
                type, oid, tournament.getTid(), sCid.cid());
        return oid;
    }
}
