...
import static org.dan.jadalnia.app.bid.BidResource.TID_SLASH_UID;
import static org.dan.jadalnia.app.festival.ParticipantMemState.BID;
...
import org.dan.jadalnia.app.festival.TournamentAccessor;
...
public class OrderResource {
    ...
    public void findMatchesByParticipants(
        ...
        tournamentAccessor.read(fid, response,
                tournament -> orderService.findMatchesByParticipants(tournament, bid1, bid2));
    }
    ...
    public void findPendingMatches(
        ...
        tournamentAccessor.read(fid, response,
                tournament -> orderService.findAllBidsOfUid(tournament, uid)
                        .stream()
                        .map((bid) -> orderService.findPendingMatchesIncludeConsole(tournament, bid))
    ...
    public void findBidPendingMatches(
        ...
        tournamentAccessor.read(fid, response,
                tournament -> orderService.findPendingMatchesIncludeConsole(tournament, bid));
    }
    ...
    public void findPlayedMatchesByMe(
        ...
        tournamentAccessor.read(fid, response,
                tournament -> orderService.findAllBidsOfUid(tournament, uid)
                        .stream()
                        .map((bid) -> orderService.findPlayedMatchesByBid(tournament, bid))
    ...
    public void findJudgedMatches(
            ...
            @PathParam(BID) Bid bid) {
        tournamentAccessor.read(fid, response,
                tournament -> orderService.findPlayedMatchesByBid(tournament, bid));
    }
    ...
    public void scoreSetAndCompleteIfWinOrLose(
        ...
        tournamentAccessor.update(score.getTid(), response,
                (tournament, batch) -> {
                    return orderService.scoreSet(tournament, uid, score, clocker.get(), batch);
                });
    }
    ...
    public void getMatchRules(
            @Suspended AsyncResponse response,
            @PathParam(TID) Fid fid) {
        tournamentAccessor.read(fid, response, tournament -> tournament.getRule().getMatch());
    }
    ...
    public void findOpenMatchesForJudge(
            @PathParam(TID) Fid fid,
            @Suspended AsyncResponse response) {
        tournamentAccessor.read(fid, response, orderService::findOpenMatchesFurJudgeIncludingConsole);
    }
    ...
    public void findOpenMatchesForWatching(
            @Suspended AsyncResponse response,
            @PathParam(TID) Fid fid) {
        tournamentAccessor.read(fid, response, orderService::findOpenMatchesForWatching);
    }
    ...
    public void getMatchForJudge(
            ...
            @PathParam(MID) Oid oid) {
        tournamentAccessor.read(fid, response,
                tournament -> orderService.getMatchForJudge(tournament, oid));
    }
}
